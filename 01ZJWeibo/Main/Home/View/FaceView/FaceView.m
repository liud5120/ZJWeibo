...
#define item_width  (kWidth/7.0)  //单个表情占用的区域宽度
...
- (void)_initData{
    ...
    frame.size.width = _items.count * kWidth;
    ...
    for (int i=0; i<_items.count; i++) {
        ...
        for (int j=0; j<item2D.count; j++) {
            ...
            /**
             ...
             通过row计算出Y坐标：row --> y
             */
            CGFloat x = colum * item_width + (item_width-face_width)/2 + i*kWidth;
...
- (void)touchFace:(CGPoint)point {
    ...
    //1.计算页数
    NSInteger page = point.x / kWidth;
    if (page >= _items.count) {
        return;
    }
    
    //2.计算row、colum
    NSInteger colum = (point.x - ((item_width-face_width)/2 + page*kWidth))/item_width;
    ...
    if (![_selectedFaceName isEqualToString:faceName]) {
        ...
        CGFloat x = colum*item_width + item_width/2 + page*kWidth;
